"use strict";

Object.defineProperty(exports, "__esModule", {
    value: true
});

var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

/**
 * @author 和霁
 * @date 2017-11-15
 */

var Arithmetic = function () {
    function Arithmetic(num) {
        _classCallCheck(this, Arithmetic);

        this.value = num;
        this.current = this._numToString(num);
    }

    /**
     * 利用字符串去除小数点
     * @param {*} arg 待处理的数据
     * @return {*} numString 去除小数点转化得到的整数
     * @return {*} decimalLen 数字精度
     * -----------------------------------------------------------------
     */


    _createClass(Arithmetic, [{
        key: "_numToString",
        value: function _numToString(arg) {
            var l = void 0,
                numString = void 0;
            try {
                numString = arg.toString();
                l = numString.split(".")[1].length;
            } catch (e) {
                l = 0;
            }

            return {
                numString: Number(numString.replace(".", "")),
                decimalLen: l
            };
        }
    }, {
        key: "add",
        value: function add(num) {
            var next = this._numToString(num);

            var _current = this.current,
                n1 = _current.numString,
                l1 = _current.decimalLen;
            var n2 = next.numString,
                l2 = next.decimalLen;


            var c = Math.abs(l1 - l2);
            var m = Math.pow(10, Math.max(l1, l2));
            //cm：俩数小数点位数之差*10
            if (c > 0) {
                var cm = Math.pow(10, c);
                l1 > l2 ? n2 = n2 * cm : n1 = n1 * cm;
            }
            this.value = (n1 + n2) / m;

            this.current = this._numToString(this.value);

            return this;
        }
    }, {
        key: "sub",
        value: function sub(num) {
            this.add(-num);
        }
    }, {
        key: "mul",
        value: function mul(num) {
            var next = this._numToString(num);

            var _current2 = this.current,
                n1 = _current2.numString,
                l1 = _current2.decimalLen;
            var n2 = next.numString,
                l2 = next.decimalLen;


            var m = l1 + l2;

            this.value = n1 * n2 / Math.pow(10, m);

            this.current = this._numToString(this.value);

            return this;
        }
    }, {
        key: "div",
        value: function div(num) {
            var next = this._numToString(num);

            var _current3 = this.current,
                n1 = _current3.numString,
                l1 = _current3.decimalLen;
            var n2 = next.numString,
                l2 = next.decimalLen;


            var m = l2 - l1;
            var c = Math.abs(m);

            this.value = m >= 0 ? n1 * Math.pow(10, m) / n2 : n1 / (n2 * Math.pow(10, c));

            this.current = this._numToString(this.value);

            return this;
        }
    }, {
        key: "getValue",
        value: function getValue() {
            return this.value;
        }
    }]);

    return Arithmetic;
}();

exports.default = Arithmetic;
//# sourceMappingURL=arithmetic.js.map